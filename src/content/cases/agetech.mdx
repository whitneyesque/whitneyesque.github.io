...
I organized the question set into tiers because a flat question list couldn't do two things at once: collect enough to make the forecast feel real and respect the limits of someone already at their limit. The tiers separated those goals. Required fields every user would answer. High-value fields that would make the output meaningfully more useful. Enrichment fields for users willing to go deeper. Diagnosis-specific subsets fired only if dementia, cancer, Parkinson's, heart disease, or other Tier 1 conditions were involved, because putting clinical language in front of users whose loved one didn't have the condition would do more harm than good. A separate caregiver layer of personality and communication-style questions shaped how the chatbot would interact with them, sitting orthogonal to the forecast inputs because tone is a different problem than content. Geographic state mattered because care costs vary by 3x across regions and the forecast was unusable without it.
...
**The conversational format had a structural problem.** During one mode, the GPT presented a numbered list of ten yes/no screening questions. The user worked through them. The conversation continued. A second numbered list appeared further down the thread. By that point the first list had scrolled off screen. When the user said "1," they were responding to the second list, but the GPT interpreted it against the first and pulled a condition that didn't apply. The conversation veered off a false premise and we couldn't recover it cleanly. This wasn't a user error. The GPT had the full conversation in memory; the user only had what was on screen. Stack sequential numbered lists in a scrolling chat window and the user loses context the model doesn't know is gone.
...
Five non-negotiable rules governed every mode.

**No Mirror.** Never reorganize a user's information and present it back as output. If the artifact doesn't contain something the user didn't already know, it has failed. This was the v1 frustration turned into a rule: reflection wasn't value.

**No Pamphlet Content.** Nothing findable in 30 seconds online. Every piece of content had to demonstrate expertise the user lacked or produce an artifact they couldn't create alone.

**Artifact-First.** The artifact was the deliverable, not the conversation. Every mode ended with a tangible output produced in full.

**No Freeform.** Every interaction followed a structured path with probes and outputs. Structure was the value.

**Invisible Emotional Support.** Never ask "How are you feeling?" or "Are you okay?" If a user shared something heavy, validate with one sentence, then continue the work. The work was the emotional container. The Pressure Valve mode was the only exception, designed specifically for moments when nothing else could move yet.
